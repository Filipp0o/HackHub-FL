...
class CreareHackathonBoundaryTest {
    ...
    void creaHackathonTramiteApiRest() throws Exception {
        ...
        assertAll(
                ...
                () -> assertEquals(
                        1L,
                        hackathonSalvato.getOrganizzatore().getId()
                ),
                () -> assertEquals(
                        2L,
                        hackathonSalvato.getGiudice().getId()
                ),
                () -> assertEquals(
                        List.of(3L, 4L),
    ...
    private static class UtenteRepositoryFinto implements UtenteRepository {
        ...
        public List<Utente> recuperaUtentiAssegnabili() {
            return List.of(
                    Utente.ricostruisci(
                            2L, "giudice@example.com", "hash"
                    ),
                    Utente.ricostruisci(
                            3L, "mentore1@example.com", "hash"
                    ),
                    Utente.ricostruisci(
                            4L, "mentore2@example.com", "hash"
                    )
            );
        }
