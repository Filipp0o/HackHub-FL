...
public class AccettareInvitoTeamControl {
    ...
    public AccettareInvitoTeamControl(
        ...
        this.partecipazioneRepository = Objects.requireNonNull(partecipazioneRepository,
    ...
    // Serializza le accettazioni gestite da questa istanza condivisa in memoria.
    public synchronized void richiediAccettazioneInvito(Utente utente, Invito invito) {
        Objects.requireNonNull(utente, "L'utente è obbligatorio");
        ...
        Invito ricevuto = richiediInvitiRicevuti(utente).stream()
                .filter(candidato -> invito.getId().equals(candidato.getId()))
                .findFirst()
    ...
    public void verificaAmmissibilitaNuovoMembro(Team team) {
        ...
        for (var partecipazione : partecipazioni) {
            if (!partecipazione.ottieniHackathon().rispettaDimensioneMassima(nuovaDimensione)) {
                throw new DimensioneMassimaSuperataException();
            }
        }
    }

    public static class InvitoNonDisponibileException extends IllegalArgumentException {
        public InvitoNonDisponibileException() {
            super("L'invito selezionato non è disponibile per l'utente");
        }
    }

    public static class UtenteGiaInTeamException extends IllegalStateException {
        public UtenteGiaInTeamException() {
            super("L'utente appartiene già a un team");
        }
    }

    public static class DimensioneMassimaSuperataException extends IllegalStateException {
        public DimensioneMassimaSuperataException() {
            super("L'ingresso supera la dimensione massima consentita per il team");
        }
    }
}
